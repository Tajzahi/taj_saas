...
async function main() {
    ...
    let tenant = await db.query.tenants.findFirst({
        where: (0, drizzle_orm_1.eq)(schema.tenants.slug, 'taj-saas'),
    });
    ...
    await db.delete(schema.orderItems).where((0, drizzle_orm_1.eq)(schema.orderItems.orderId, schema.orderItems.orderId)); // delete all items
    await db.delete(schema.orders).where((0, drizzle_orm_1.eq)(schema.orders.tenantId, tenantId));
    await db.delete(schema.shiftLogs).where((0, drizzle_orm_1.eq)(schema.shiftLogs.tenantId, tenantId));
    await db.delete(schema.shifts).where((0, drizzle_orm_1.eq)(schema.shifts.tenantId, tenantId));
    await db.delete(schema.inventoryTransactions).where((0, drizzle_orm_1.eq)(schema.inventoryTransactions.tenantId, tenantId));
    await db.delete(schema.inventory).where((0, drizzle_orm_1.eq)(schema.inventory.tenantId, tenantId));
    await db.delete(schema.recipeIngredients).where((0, drizzle_orm_1.eq)(schema.recipeIngredients.recipeId, schema.recipeIngredients.recipeId)); // delete all
    await db.delete(schema.recipes).where((0, drizzle_orm_1.eq)(schema.recipes.tenantId, tenantId));
    await db.delete(schema.menuItems).where((0, drizzle_orm_1.eq)(schema.menuItems.tenantId, tenantId));
    await db.delete(schema.categories).where((0, drizzle_orm_1.eq)(schema.categories.tenantId, tenantId));
    await db.delete(schema.approvals).where((0, drizzle_orm_1.eq)(schema.approvals.tenantId, tenantId));
    await db.delete(schema.profiles).where((0, drizzle_orm_1.eq)(schema.profiles.tenantId, tenantId));
    await db.delete(schema.branches).where((0, drizzle_orm_1.eq)(schema.branches.tenantId, tenantId));
    ...
    for (const sdm of sdmData) {
        ...
        await db.delete(schema.session).where((0, drizzle_orm_1.eq)(schema.session.userId, sdm.id));
        await db.delete(schema.account).where((0, drizzle_orm_1.eq)(schema.account.userId, sdm.id));
        await db.delete(schema.user).where((0, drizzle_orm_1.eq)(schema.user.id, sdm.id));
        // Insert user
    ...
    for (const item of parsedMenuItems) {
        const categoryObj = categoriesMap[item.category] || catMartabakTelur;
