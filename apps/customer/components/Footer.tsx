"use client";
import Link from 'next/link';
import { MapPin, Clock } from 'lucide-react';

type LayoutSettings = {
  store_name: string;
  logo_url: string | null;
  primary_color: string;
  secondary_color: string;
  store_address: string;
  opening_hours: string;
  tagline?: string;
  social_links?: {
    instagram?: string;
    facebook?: string;
    tiktok?: string;
    youtube?: string;
  };
};

// Normalisasi URL sosmed — handle jika hanya username (bukan URL penuh)
function normalizeSocialUrl(value: string | undefined, platform: 'instagram' | 'facebook' | 'tiktok' | 'youtube'): string {
  if (!value) {
    const defaults: Record<string, string> = {
      instagram: 'https://instagram.com',
      facebook: 'https://facebook.com',
      tiktok: 'https://tiktok.com',
      youtube: 'https://youtube.com',
    };
    return defaults[platform];
  }
  // Sudah URL lengkap
  if (value.startsWith('http://') || value.startsWith('https://')) return value;
  // Hanya username — tambah prefix
  const prefix: Record<string, string> = {
    instagram: 'https://instagram.com/',
    facebook: 'https://facebook.com/',
    tiktok: 'https://tiktok.com/@',
    youtube: 'https://youtube.com/@',
  };
  return `${prefix[platform]}${value.replace(/^@/, '')}`;
}

export default function Footer({ settings }: { settings?: LayoutSettings }) {
  const storeName = settings?.store_name || "";
  const address = settings?.store_address || "";
  const hours = settings?.opening_hours || "";
  const tagline = settings?.tagline || "";

  const primaryColor = settings?.primary_color || '#8E0E0E';
  const secondaryColor = settings?.secondary_color || '#E05009';

  return (
    <footer className="bg-[#1a0a0a] text-white pt-12 pb-6">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-10">
          {/* Brand */}
          <div className="lg:col-span-2">
            <div className="flex items-center gap-3 mb-4">
              <div className="w-12 h-12 rounded-full flex items-center justify-center flex-shrink-0 text-white font-black text-xl shadow-md overflow-hidden" style={{ background: `linear-gradient(to bottom right, ${primaryColor}, ${secondaryColor})` }}>
                {settings?.logo_url ? (
                  <img src={settings.logo_url} alt={storeName || "Logo"} className="w-full h-full object-cover" />
                ) : (
                  <span>{storeName ? storeName.charAt(0).toUpperCase() : "🏪"}</span>
                )}
              </div>
              <div>
                <p className="font-bold text-xl text-white uppercase">{storeName}</p>
                {tagline && <p className="text-sm text-gray-400">{tagline}</p>}
              </div>
            </div>
            <p className="text-gray-400 text-sm leading-relaxed mb-4 max-w-sm">
              Cita rasa otentik dan sajian berkualitas tinggi. 
              Dibuat dengan bahan pilihan dan dedikasi terbaik untuk pelanggan setia kami.
            </p>
            <div className="space-y-2">
              {address && (
                <div className="text-sm text-gray-300 flex items-start gap-2">
                  <MapPin className="w-4 h-4 shrink-0 mt-0.5" style={{ color: secondaryColor }} />
                  <div>
                    <p className="text-white font-medium">{address}</p>
                  </div>
                </div>
              )}
              {hours && (
                <p className="text-sm text-gray-300 flex items-center gap-2">
                  <Clock className="text-[#E05009] w-4 h-4 shrink-0" />
                  <span>{hours}</span>
                </p>
              )}
              <p className="text-sm text-gray-300 flex items-center gap-2">
                <img src="/Halal logo.jfif" alt="Halal" className="w-5 h-5 object-contain rounded bg-white p-0.5" />
                <span>Halal Certified</span>
              </p>
            </div>
          </div>

          {/* Navigasi Cepat */}
          <div>
            <p className="font-bold text-white mb-4 text-base">Menu & Layanan</p>
            <ul className="space-y-2 text-sm text-gray-400">
              <li>
                <Link href="/menu" className="hover:text-[#E05009] transition-colors">
                  Daftar Menu
                </Link>
              </li>
              <li>
                <Link href="/catering" className="hover:text-[#E05009] transition-colors">
                  Paket Katering
                </Link>
              </li>
              <li>
                <Link href="/promo" className="hover:text-[#E05009] transition-colors">
                  Promo & Kupon
                </Link>
              </li>
              <li>
                <Link href="/tracking" className="hover:text-[#E05009] transition-colors">
                  Lacak Pesanan
                </Link>
              </li>
              <li>
                <Link href="/gallery" className="hover:text-[#E05009] transition-colors">
                  Galeri Foto
                </Link>
              </li>
            </ul>
          </div>

          {/* Bantuan & Info */}
          <div>
            <p className="font-bold text-white mb-4 text-base">Informasi</p>
            <ul className="space-y-2 text-sm text-gray-400 mb-4">
              <li>
                <Link href="/about" className="hover:text-[#E05009] transition-colors">
                  Tentang Kami
                </Link>
              </li>
              <li>
                <Link href="/faq" className="hover:text-[#E05009] transition-colors">
                  Tanya Jawab (FAQ)
                </Link>
              </li>
              <li>
                <Link href="/contact" className="hover:text-[#E05009] transition-colors">
                  Hubungi Kami
                </Link>
              </li>
            </ul>

            {/* Social Icons */}
            <div className="flex gap-2 pt-2">
              <a
                href={normalizeSocialUrl(settings?.social_links?.instagram, 'instagram')}
                target="_blank"
                rel="noopener noreferrer"
                aria-label="Instagram"
                className="w-9 h-9 rounded-lg bg-white/10 hover:bg-[#E05009] flex items-center justify-center transition-colors"
              >
                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/></svg>
              </a>
              <a
                href={normalizeSocialUrl(settings?.social_links?.facebook, 'facebook')}
                target="_blank"
                rel="noopener noreferrer"
                aria-label="Facebook"
                className="w-9 h-9 rounded-lg bg-white/10 hover:bg-[#E05009] flex items-center justify-center transition-colors"
              >
                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
              </a>
              <a
                href={normalizeSocialUrl(settings?.social_links?.tiktok, 'tiktok')}
                target="_blank"
                rel="noopener noreferrer"
                aria-label="TikTok"
                className="w-9 h-9 rounded-lg bg-white/10 hover:bg-[#E05009] flex items-center justify-center transition-colors"
              >
                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M19.589 6.686a4.793 4.793 0 0 1-3.77-4.245V2h-3.445v13.672a2.896 2.896 0 1 1-2.896-2.896c.244 0 .478.033.702.091V9.387a6.34 6.34 0 0 0-.702-.039 6.34 6.34 0 1 0 6.34 6.34V8.724a8.214 8.214 0 0 0 4.771 1.517V6.796a4.805 4.805 0 0 1-1.000-.11z"/></svg>
              </a>
            </div>
          </div>
        </div>

        {/* Legal & Copyright */}
        <div className="border-t border-white/10 pt-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-sm text-gray-500">
            © {new Date().getFullYear()} {storeName}. All rights reserved.
          </p>
          <div className="flex gap-4">
            <Link href="/privacy" className="text-sm text-gray-500 hover:text-[#E05009] transition-colors">
              Kebijakan Privasi
            </Link>
            <Link href="/terms" className="text-sm text-gray-500 hover:text-[#E05009] transition-colors">
              Syarat & Ketentuan
            </Link>
          </div>
        </div>
      </div>
    </footer>
  );
}
