    ...
    <div ref={containerRef} className={`relative w-full ${open ? 'z-30' : 'z-10'}`}>
      ...
      {open && (
        <div
          ...
          className="absolute top-full right-0 mt-1 z-[100] w-52 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl shadow-2xl max-h-[180px] overflow-y-auto overscroll-contain divide-y divide-slate-100 dark:divide-slate-800 animate-fade-in"
        >
          {filtered.length > 0 ? (
            filtered.map((item) => (
              <button
