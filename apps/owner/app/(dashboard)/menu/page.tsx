    ...
    <div ref={containerRef} className={`relative w-full ${open ? 'z-30' : 'z-10'}`}>
      <div className="relative flex items-center">
        <input
          ...
          placeholder="gr/ml"
          className="w-full text-xs px-2 py-1.5 pr-6 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200 focus:outline-none focus:border-orange-500 font-medium"
          required
        />
        <button
          type="button"
          ...
          className="absolute right-1 text-[10px] text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 p-1 cursor-pointer"
        >
          ▼
        </button>
      </div>
      ...
      {open && (
        <div
          ...
          className="absolute top-full left-0 mt-1 z-[100] w-52 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl shadow-2xl max-h-[220px] overflow-y-auto overscroll-contain divide-y divide-slate-100 dark:divide-slate-800 animate-fade-in"
        >
          {filtered.length > 0 ? (
            filtered.map((item) => (
              <button
